...
proof -
  ...
  hence 0:
    "\<forall>A p. finite_profile A p \<longrightarrow>
        set_equals_partition A (drop_module n r A p vs)"
    by simp
  have
    "\<forall>A p. finite_profile A p \<longrightarrow>
  ...
  hence
    "\<forall>A p. finite_profile A p \<longrightarrow>
        ...
        {a \<in> A. card(above (limit A r) a) > n} = {}"
    by blast
  hence 1: "\<forall>A p vs. finite_profile A p \<longrightarrow> disjoint3 (?mod A p vs)"
    by simp
  from 0 1 have
    "\<forall>A p vs. finite_profile A p \<longrightarrow>
        well_formed A (?mod A p vs)"
    by simp
  hence
    "\<forall>A p vs. finite_profile A p \<longrightarrow>
        well_formed A (?mod A p vs)"
