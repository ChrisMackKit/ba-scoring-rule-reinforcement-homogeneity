...
lemma cond_winner_imp_copeland_score:
  assumes winner: "condorcet_winner A p w"
  shows "copeland_score w A p vs = card A -1"
...
proof (safe)
  ...
  have "well_formed A (max_eliminator copeland_score A p vs)"
    ...
    by blast
  thus "well_formed A (copeland A p vs)"
    by simp
next
  fix
    A :: "'a set" and
  ...
  have
    "\<forall>A p. (copeland A p vs = max_eliminator copeland_score A p vs)"
    by simp
