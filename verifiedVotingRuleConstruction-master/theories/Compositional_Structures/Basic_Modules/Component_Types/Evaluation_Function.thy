...
type_synonym 'a Evaluation_Function = "'a  \<Rightarrow> 'a set \<Rightarrow> 'a Profile \<Rightarrow> 'a Pair_Vectors \<Rightarrow> nat"
...
definition condorcet_rating :: "'a Evaluation_Function \<Rightarrow> bool" where
  "condorcet_rating f \<equiv>
    \<forall>A p w vs. condorcet_winner A p w \<longrightarrow>
      (\<forall>l \<in> A . l \<noteq> w \<longrightarrow> f l A p vs < f w A p vs)"
  ...
  *)
  ...
  proof (safe)
    ...
    have "\<forall>n na. (n::nat) \<noteq> na \<or> n \<le> na"
      by simp
    with aInA show "e a A p vs \<le> e w A p vs"
...
theorem non_cond_winner_not_max_eval:
  assumes
    ...
    linA: "l \<in> A" and
    loser: "w \<noteq> l"
  shows "e l A p vs < Max {e a A p vs | a. a \<in> A}"
proof -
  have "e l A p vs < e w A p vs"
    using condorcet_rating_def linA loser rating winner
    by metis
  also have "e w A p vs = Max {e a A p vs |a. a \<in> A}"
