...
next
  ...
  hence "reject (pass_module n r) A p vs \<noteq> A"
    by simp
  thus "False"
    using order card_A
    by simp
...
next
  ...
  assume
    ...
    prof_A: "profile A p"
  show
    "card (defer (pass_module 0 r) A p vs) = 0"
  proof -
    have lin_ord_on_A:
      "linear_order_on A (limit A r)"
    ...
    hence "card {a \<in> A. card(above (limit A r) a) \<le> 0} = 0"
      using card.empty
      by metis
    thus "card (defer (pass_module 0 r) A p vs) = 0"
      by simp
  qed
qed
...
next
  ...
  assume
    ...
    prof_A: "profile A p"
  show
    "card (defer (pass_module 1 r) A p vs) = 1"
  proof -
    have "A \<noteq> {}"
      using card_pos
      by auto
    ...
    proof
      ...
      thus ?thesis
        by simp
    qed
    hence "defer (pass_module 1 r) A p vs = {w}"
      by simp
    thus "card (defer (pass_module 1 r) A p vs) = 1"
      by simp
  qed
qed
...
next
  ...
  from a have "a \<in> {a \<in> A. card(above (limit A r) a) \<le> 2}"
    ...
    by (metis (no_types, lifting))
  hence a_in_defer: "a \<in> defer (pass_module 2 r) A p vs"
    by simp
  have "finite (A-{a})"
    by (simp add: finA)
  ...
  hence card_above_b_eq_2: "card (above (limit A r) b) = 2"
    ...
    by auto
  hence b_in_defer: "b \<in> defer (pass_module 2 r) A p vs"
  ...
  hence "\<forall>c \<in> A-{a, b}. card (above (limit A r) c) > 2"
    ...
    by metis
  hence "\<forall>c \<in> A-{a, b}. c \<notin> defer (pass_module 2 r) A p vs"
    by (simp add: not_le)
  moreover have "defer (pass_module 2 r) A p vs \<subseteq> A"
    by auto
  ultimately have "defer (pass_module 2 r) A p vs \<subseteq> {a, b}"
    by blast
  with a_in_defer b_in_defer have
    "defer (pass_module 2 r) A p vs = {a, b}"
    by fastforce
  thus "card (defer (pass_module 2 r) A p vs) = 2"
