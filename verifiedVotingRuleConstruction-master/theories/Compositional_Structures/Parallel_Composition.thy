...
fun parallel_composition :: "'a Electoral_Module \<Rightarrow> 'a Electoral_Module \<Rightarrow>
  ...
  "parallel_composition m n agg A p vs = agg A (m A p vs) (n A p vs)"
...
proof (safe)
  ...
  have "well_formed A (a A (m A p vs) (n A p vs))"
    using agg_a combine_ele_rej_def fin_A
          mod_m mod_n prof_A wf_imp wf_quant vec_A
    by metis
  thus "well_formed A ((m \<parallel>\<^sub>a n) A p vs)"
    by simp
qed
...
next
  ...
  hence
    ...
    by presburger
  hence
    "let c = (a A (m A p vs) (n A p vs)) in
      (elect_r c \<subseteq> ((elect m A p vs) \<union> (elect n A p vs)))"
    using emod_m emod_n fin_A par_comp_result_sound
          prod.collapse prof_A vec_A
    by metis
  hence "x \<in> ((elect m A p vs) \<union> (elect n A p vs))"
    using x_wins
    by auto
  thus "x \<in> {}"
    using sup_bot_right non_electing_def fin_A
          non_electing_m non_electing_n prof_A vec_A
