...
fun pairwise_majority_rule :: "'a Electoral_Module" where
  "pairwise_majority_rule A p vs = elector condorcet A p vs"

fun condorcet' :: "'a Electoral_Module" where
"condorcet' A p vs =
  ((min_eliminator condorcet_score) \<circlearrowleft>\<^sub>\<exists>\<^sub>!\<^sub>d) A p vs"

fun pairwise_majority_rule' :: "'a Electoral_Module" where
"pairwise_majority_rule' A p vs = iterelect condorcet' A p vs"
